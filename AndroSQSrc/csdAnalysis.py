    ...
    def VerifyKeyComponent(self, cur_method, key_component_set):
        ...
        for key_component in key_component_set:        
            if key_component == "Service":
                ...
                if launchActivity == None:
                    return None, None
                if launchActivity!=None and launchActivity.find(".")>-1:
                ...
                self.d.get_class(cur_classname).get_superclassname().find("BroadcastReceiver")>-1:
                    ...
                    return key , key_component                 
                elif self.d.get_class(cur_classname).get_interfaces()!= None and \
                self.d.get_class(cur_classname).get_interfaces().find("BroadcastReceiver")>-1  : 
                     # Dynamic register
                    #print "[verify] verified method(BroadcastReceiver): " + key +"\n"
                    return key , key_component           
                
            #elif key_component == "$":
                #if cur_classname.find("$")>-1:
                    ...
                    #big_class_name = cur_classname[0:cur_classname.find("$")-1]
                    #print "[verify] big_class_name(Inner class): " + big_class_name +"\n"
                    #if self.d.get_class(big_class_name)!= None:
                        
                        #ret_key_tmp, ret_key_component = self.VerifyKeyComponent(None,key_component_set, big_class_name)
                        
                    #return key, ret_key_component + "$" 
                ##elif self.d.get_class(cur_classname).get_superclassname().find("$")>-1 :
                    ##print "[verify] verified method(Inner class): " + key +"\n"
                
            elif key_component == "SensorEventListener":
                
                if self.d.get_class(cur_classname).get_interfaces()!=None and \
                self.d.get_class(cur_classname).get_interfaces().find("SensorEventListener")>-1:
                    #print "[verify] verified method(SensorEventListener): " + key +"\n"
                    return key , key_component  
                
            #elif key_component == "Listener":
                            
                #if self.d.get_class(cur_classname).get_interfaces()!=None and \
                #self.d.get_class(cur_classname).get_interfaces().find("Listener")>-1:
                    
                    #print "[verify] verified method(Listener): " + key +"\n"
                    #return key , key_component             
...
def Main_If_Servicewithsink(apk_d_inputDex, key_component_set= csdConf.first_state_class, observable_judge = True):
    ret = []
    ...
    activity_num = len(apk.get_activities())
    service_num = len (apk.get_services())
    receiver_num = len (apk.get_receivers())
    provider_num = len (apk.get_providers())
    
    #with open (csdConf.mtrace_result_path, mode='a')as f:
        #f.write("====================================\n")
        #f.write("[APK] "+ str(apk.get_package()) +"\n")
        #f.write("====================================\n")
        #f.write("[acitivity_num]" + str(activity_num)+"\n")
        #f.write("[service_num]"+ str(service_num)+"\n")
        #f.write("[broadcastreceiver_num]" + str(receiver_num)+"\n")
        #f.write("[provider_num]" + str(provider_num)+"\n")     
    ...
    with open (csdConf.mtrace_result_path, mode='a')as f:
        ...
        for item in ret:
            f.write("[SINK] "+item[1]["class"]+ ":" +item[1]["method"]+"\n")
            for iitem in item[2]:
                f.write("[method_path] "+ iitem.get_class_name()+":"+iitem.get_name() +"\n")
            f.write("[key_component] " + item[3] +"\n")
        f.write("\n\n")
    ...
    if len(sinkpremethods)>0:
        for singlesinkpre in sinkpremethods:
            ...
            for i in singlesinkpre:
                ...
                b = c.BackTrace(i)
                c.PrintBacktrace(b)
                #c.SaveBacktrace(b,csdConf.mtrace_result_path)
...
if __name__ == "__main__":
    ...
    c,ret =  Main_If_Servicewithsink((apk,d,inputDex),csdConf.first_state_class)
